...
### Подготовка
 Требуемая минимальная конфигурация для работы **IW4MAdmin**. Необходимо только одно условие.  
...
3. Введите `dotnet IW4MAdmin.dll` и нажмите *Enter*, или же для _Windows_ создайте *_bat_* или *_cmd_* файл с таким содержимым:
...
**Все команды, которые нуждаются в `цели` смотрят на `первый аргумент` в форме идентификации игрока**
...
### Разное
#### Базы данных
Вся информация **IW4MAdmin** сохраняется в файле `Database.db`. Если вам нужно сбросить всю базу данных, то этот файл можно просто удалить. Кроме того, этот файл сохраняется во время обновлений, чтобы сохранить всю информацию о клиентах.

...
* Показывает ссылку на социальные сети/веб-сайт вашего сообщества в Веб-Интерфейсе.
* По умолчанию &mdash; `false`
...
* **Русскоязычные пользователи для отображения кириллицы должны использовать ** `windows-1251` ** и указать её в качестве кодировки текста**
* По умолчанию &mdash; `false`
...
`Введите IP сервера`
* Почти для всех сценариев подходит `127.0.0.1`
...
`CustomLocale`
* Указывает [локализацию](https://msdn.microsoft.com/en-us/library/39cwe7zf.aspx) которая будет использоваться вместо системной по умолчанию
* Локализация должна быть из `Equivalent Locale Name` столбца
...
- Этот плагин определяет использует ли клиент VPN, если да то исключает его из сервера
...
#### Исключение общего GUID [Скрипт плагин]
...
#### NuGet пакет
Пакет NuGet для "Общей библиотеки" **IW4MAdmin** можно получить в [NuGet галерее](https://www.nuget.org/packages/RaidMax.IW4MAdmin.SharedLibraryCore)
